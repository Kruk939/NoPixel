...
diag_log "SERWER: Pomyślnie ustawiłem startowe ceny surowców";
 ...
 while {true} do {
    //if (initEconomy == 0) exitWith{initEconomy = 1; publicVariable "initEconomy"; diag_log "Closed 1 of 2 theard (fn_economyEvents.sqf)";};
    ...
    //if (_chance isEqualTo 1) then { priceArrayOre set [0, 85]; publicVariable "priceArrayOre"; ["Górnicy robią strajk, w zakładach brakuje brązu do produkcji!", false] remoteExec ["domsg",-2]; diag_log "SERWER: Pomyślnie wykonano economyEvent: Copper"; };
    switch (_selected) do {
