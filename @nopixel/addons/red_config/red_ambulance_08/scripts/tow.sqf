client_fnc_towvehicle = {
	...
	_memes = 0;
	if (typeOf _car == "A3L_Stretcher_F") then {_car attachTo [(vehicle player), [0,-2,2] ]; _memes = 1;  };
	
	if (_memes == 0) exitWith {hint "This is not a Stretcher!"};
...
client_fnc_untowvehicle = {
	...
	if (isNull _car) exitWith {hint "No stretcher attached"};
	vehicle player allowDamage false;
	detach _car;
