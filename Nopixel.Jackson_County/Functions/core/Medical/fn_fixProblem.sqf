...
if(_arrayCount == 10) then {
	if(_amount > 0) then { if(myJob == "EMS" || myjob == "Fire") then { ["Podaję antybiotyk",5,client_fnc_healTime,_user,"AinvPknlMstpSnonWnonDnon_medic_1",[_user,_myInjuries,_arrayCount,"CG_ATF_Bandage_i"],"cg_mission_files\sounds\patdown1.ogg"] spawn client_fnc_dotask; } else { hint "You dont have the required item!"; }; };
};	
closedialog 0;
sleep 3;
imhealing = false;
