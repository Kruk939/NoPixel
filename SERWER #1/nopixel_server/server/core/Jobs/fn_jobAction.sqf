...
if(currentMailmen isequalto []) exitwith { [] remoteExec ["StanLakeside_fnc_completemail",_sender]; }; 
	_units = currentMailmen; 
	_selection = _units call BIS_fnc_selectRandom; 
[_location,_sender,_jobType,_message] remoteExec ["StanLakeside_fnc_mailSystem",_selection]; 
}; 
...
_selection = _units call BIS_fnc_selectRandom; 
[_location,_jobType] remoteExec ["StanLakeside_fnc_startNews",_selection]; 
}; 
