...
diag_log format["DEBUG - StanLakesideServer_fnc_updateCarStatus: %1",_this];
...
if (_status isEqualTo 0) then {
	 ...
	 if (_player in currentCop) then {
		[_player,objNull,18,format ["%1 odholował %2 (%3)", name _player, _vehicleName, _className],_className] call StanLakesideServer_fnc_copLog;
	 } else {
		[_player,4,format ["%1 zezlomowal %2", name _player, _vehicleName],"",_className,_vehicleName] call StanLakesideServer_fnc_vehiclelog;
	 };
...
if (_status isEqualTo 0) then {
	...
	_damage = damage _object;
	 if !(_damage == 1) then {
		...
		[_fuel,_damage,_license] call StanLakesideServer_fnc_updateFuelDamage;
