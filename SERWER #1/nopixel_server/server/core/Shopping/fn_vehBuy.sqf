...
if((count _information) isEqualTo 0) exitWith { [_buyer,6,format ["%1 nie mógł kupić samochodu i stracił %2",name _buyer,_price],_price,"",""] call StanLakesideServer_fnc_vehicleLog; };
...
if(isNull _player) then {
	...
	[_price, _veh] remoteExec["StanLakesideClient_fnc_vehSold", _player];
};
["garage",_information] remoteExec ["StanLakesideClient_fnc_setVariable", _buyer];
...
[player,6,format ["%1 kupił pojazd %2 na kwotę %3 od gracza %4",name player,_vehicleName,name _player],_price,_className,_vehicleName] call StanLakesideServer_fnc_vehicleLog;
deleteVehicle _veh;