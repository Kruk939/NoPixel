...
if (_statementsent == 1) then {
	...
	[player,_target,1,format ["%1 teleportował się do %2",name player, name _target],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
};
...
if (_statementsent == 2) then {
	...
	[player,_target,2,format ["%1 teleportował %2 do siebie",name player, name _target],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
};
...
if(_statementsent == 5) then {
	...
	[player,_target,5,format ["%1 włączył niewidzialność dla %2",name player, name _target],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
};

if(_statementsent == 6) then {
	[_target,false] remoteExec ["hideobjectglobal", 2];
	player setVariable ["st_hide",false, true];
	[player,_target,6,format ["%1 wyłączył niewidzialność dla %2",name player, name _target],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
};
...
if(_statementsent == 8) then {
	[] remoteExec ["StanLakesideServer_fnc_statsaveloop", 2];
	["Wykonałeś synchronizację wszystkich graczy!", true] spawn domsg;
	[player,objNull,8,format ["%1 wykonał synchronizację graczy",name player],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
};

if(_statementsent == 9) then {
	true remoteExec ["disableuserinput", _target]; 
	[player,_target,9,format ["%1 wyłączył sterowanie dla %2",name player, name _target],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
};

if(_statementsent == 10) then {
	false remoteExec ["disableuserinput", _target]; 
	[player,_target,10,format ["%1 włączył sterowanie dla %2",name player, name _target],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
};
...
if(_statementsent == 19) then {
	...
	[player,objNull,19,format ["%1 ustawił pobliskim (300m) graczom swój ekwipunek (%2)",name player, _loadout],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
};
...
if(_statementsent == 20) then {
	...
	[player,objNull,20,format ["%1 przeteleportował wszystkich graczy z 300m do siebie",name player],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
};
...
if(_statementsent == 23) then {
	...
	{
		_veh lock 0;
		[player,_target,23,format ["%1 otworzył pojazd gdzie znajdował się gracz %2",name player, name _target],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
	} 
		else 
	{
		_veh lock 2;
		[player,_target,24,format ["%1 zamknął pojazd gdzie znajdował się gracz %2",name player, name _target],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
	};
};
...
if(_statementsent == 24) then {
	...
	if(!Ghosting) exitwith {
		...
		while{Ghosting} do {
			if(vehicle _target == _target) then {
				if(player IN (attachedobjects _target)) then { sleep 3; } else { player attachTo [_target, [0, 0, 1] ]; sleep 5; [player,_target,25,format ["%1 rozpoczął ghostSpectate na graczu %2",name player, name _target],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];};
			};

			if(vehicle _target != _target) then {
				if(player IN (attachedobjects vehicle _target)) then { sleep 3; } else { player attachTo [vehicle _target, [0, 0, 1] ]; sleep 5; [player,_target,25,format ["%1 rozpoczął ghostSpectate na graczu %2",name player, name _target],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];};
			};
		};
	};
	if(Ghosting) exitwith { detach player; Ghosting = False; [player,objNull,26,format ["%1 skończył ghostSpectate %2",name player],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];};
};

if(_statementsent == 25) then {
	["Wyrzuciłeś wszystkich graczy z serwera!", true] spawn domsg;
	[player,objNull,31,format ["%1 wyrzucił z gry wszystkich graczy",name player],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
...
if(_statementsent == 26) then {
	...
	if(!LoggedIn) exitwith { 
		onMapSingleClick "if (loggedIn) then {vehicle player setPos _pos; [player,objNull,29,format [""%1 przeteleportował się na pozycję %2"",name player, _pos],_pos] remoteExec [""StanLakesideServer_fnc_adminLog"", 2];};";
		LoggedIn = true;
		[player,objNull,27,format ["%1 włączył toggleTP",name player],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
	};
	if(LoggedIn) exitwith { LoggedIn = False; [player,objNull,28,format ["%1 wyłączył toggleTP",name player],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];};
};

if(_statementsent == 27) then {
	[player,_target,30,format ["%1 wyrzucił z gry %2",name player,name _target],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
...
if(_statementsent == 29) then {
	...
	[player,objNull,35,format ["%1 otworzył Arsenal",name player],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
};

if(_statementsent == 30) then {
	closeDialog 0;
	createDialog "RscDisplayDebugPublic";
	[player,objNull,36,format ["%1 otworzył Debug Console",name player],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
};
...
if(_statementsent == 35) then {
	closeDialog 0;
	[] call BIS_fnc_camera;
	[player,objNull,41,format ["%1 otworzył kamerę",name player],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
};
...
if(_statementsent == 37) then {
	if (kif_admin<=4) then {
		adminInteractions = 1;
		[player,objNull,43,format ["%1 włączył interakcje administratora stopień 1",name player],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
	};
	if (kif_admin==5) then {
		adminInteractions = 2;
		[player,objNull,44,format ["%1 włączył interakcje administratora stopień 2",name player],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
	};
};

if(_statementsent == 38) then {
	adminInteractions = 0;
	[player,objNull,45,format ["%1 wyłączył interakcje administratora",name player],""] remoteExec ["StanLakesideServer_fnc_adminLog", 2];
};
