//	[_location,_sender,_jobType,_message] remoteExec ["StanLakeside_fnc_mailSystem",_player];
		...
		while{taskrunning && myjob == "mail"} do {
			...
			if(playertasks isequalto []) then {
				...
				if(player distance [7151.34,2462.99,0.00143814] > 100) then {
					[[7151.34,2462.99,0.00143814]] call StanLakeside_fnc_jobMarker;
				...
				if(player distance ((playertasks select 0) select 0) < 15) then {
					...
					if(((playertasks select 0) select 3) == "Personal") then {
						[] remoteExec ["StanLakeside_fnc_completemail",((playertasks select 0) select 1)];
					};
			...
			
			{
				if((_x select 3) == "Personal") then {
					[] remoteExec ["StanLakeside_fnc_completemail",(_x select 1)];
				};
			} foreach playertasks;

			if(myjob == "mail") then { [] call StanLakeside_fnc_jobEnd; };
	};
};
