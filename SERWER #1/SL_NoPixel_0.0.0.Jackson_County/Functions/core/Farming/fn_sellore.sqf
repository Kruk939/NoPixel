...
_cashTotal = 0;
{
	...
	while{ _i > 0 } do {
		...
		if(_localProtection != globalProtection) exitwith { [1,"Selling Ore Script"] spawn StanLakeside_fnc_anticheat; };

		player removeitem (_barArray select _n);
		_value = (priceArrayOre select _n);
		[_value] call StanLakeside_fnc_sl_addCash_secure;
...
["Sukces",format["Sprzedałeś surowce za: $%1", _cashTotal],[0,255,0,1],""] call StanLakeside_fnc_showNotification;
