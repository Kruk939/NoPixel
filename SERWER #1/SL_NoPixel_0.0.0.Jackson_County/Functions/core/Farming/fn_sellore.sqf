...
_cashTotal = 0;
{
	...
	while{ _i > 0 } do {
		...
		if(_localProtection != globalProtection) exitwith { [1,"Selling Ore Script"] spawn StanLakesideClient_fnc_anticheat; };

		player removeitem (_barArray select _n);
		_value = (priceArrayOre select _n);
		[_value] call StanLakesideClient_fnc_addCash;
...
["Sukces",format["Sprzedałeś surowce za: $%1", _cashTotal],[0,255,0,1],""] call StanLakesideClient_fnc_showNotification;
[player,objNull,3,format ["%1 sprzedał SUROWIEC w ilości %2 za %3 $", name player, _total, _cashTotal],_cashTotal, "SUROWIEC", _total] remoteExec ["StanLakesideServer_fnc_economyLog", 2];
