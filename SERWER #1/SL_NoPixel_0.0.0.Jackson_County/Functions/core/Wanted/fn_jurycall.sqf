...
// 
// [name player, getpos player, player] spawn StanLakesideClient_fnc_jurycall;
...
["Zostales wyznaczony do lawy przysieglych, otrzymasz $2000 jeśli proces dobiegnie końca. Czy sie zgadzasz?"] call StanLakesideClient_fnc_questionPlayer;
...
if(JuryDuty) then { 
	...
	[_location] call StanLakesideClient_fnc_jobMarker;
...
if(!_failed) then {
	...
	[getpos _officer] call StanLakesideClient_fnc_jobMarker;

	[format["Nazwisko Twojego oficera to %1, odbierz od niego $2000.",_policeOfficerName], false] spawn domsg;
	while{JuryDuty} do {
		if( clientarrested || imRestrained || myJob != "None" ) exitwith {};
		if(player distance _officer < 5) exitwith { ["Dotarles do sadu", false] spawn doquickmsg; [2000] call StanLakesideClient_fnc_addCash; }; 
		uisleep 1;
