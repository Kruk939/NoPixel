...
if (_amount > 0) then 
{
	...
	if(_bankType == 0) then {
		...
		if (_check) then 
		{
			...
			[_amount] call StanLakesideClient_fnc_addCash;

			["Sukces","Pomyślnie wypłacono pieniądze!",[0,255,0,1],""] call StanLakesideClient_fnc_showNotification;
			...
			["Błąd","Za mały stan konta!",[255,0,0,1],""] call StanLakesideClient_fnc_showNotification;
 ...
 else  
{
	...
	["Błąd","Musisz wypłacić więcej niż $0!",[255,0,0,1],""] call StanLakesideClient_fnc_showNotification;
