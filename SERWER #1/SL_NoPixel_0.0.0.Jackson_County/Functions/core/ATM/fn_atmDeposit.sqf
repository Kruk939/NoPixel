...
if (_amount > 0) then 
{
	...
	if(_bankType == 0) then {
		...
		_check = [1, _amount] call StanLakeside_fnc_sl_checkMoney_secure;
		if (_check) then 
		{
		
			[_amount] call StanLakeside_fnc_sl_removeCash_secure;
			[_amount] call StanLakeside_fnc_sl_addBank_secure;
			["Sukces","Pomyślnie wpłacono pieniądze",[0,255,0,1],""] call StanLakeside_fnc_showNotification;
			...
			["Błąd","Nie masz wystarczająco dużo gotówki!",[255,0,0,1],""] call StanLakeside_fnc_showNotification;
		...
		if (_check) then 
		{
			...
			["Błąd","Stan konta zbyt mały!",[255,0,0,1],""] call StanLakeside_fnc_showNotification;
...
{
	["Błąd","Wpłata musi wynosić więcej niż $0!",[255,0,0,1],""] call StanLakeside_fnc_showNotification;
