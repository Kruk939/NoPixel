...
if(_error == "") then {
	...
	[_amount] spawn StanLakesideClient_fnc_addCash;
	[format["Okradłeś sklep, zabrałeś $%1", _amount], false] call domsg;
	["Remove","Karma",50] call StanLakesideClient_fnc_sustain;
	...
	if(_chance < 95) then {
		[player, _shop, "storeRobbery"] spawn StanLakesideClient_fnc_createEvidence;
	};
