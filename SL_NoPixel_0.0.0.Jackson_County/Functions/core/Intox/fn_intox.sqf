[] spawn {
    uiSleep 150;
		...
		if(client_intox <= 0.02) then {client_intox = 0.00;} else {client_intox = client_intox - 0.02;};
			switch(true) do {
				case (client_intox isEqualTo 0.00): {["Jestes trzezwy.", false] spawn domsg;};
				case (client_intox isEqualTo 0.08): {["Mozesz juz prowadzic pojazd.", false] spawn domsg;};
			};
		};
};
	...
	while{client_intox > 0.08} do
	{
		...
		if(client_intox > 0.35) then {
			_chance = round (random 50);
   			 	if(_chance < 7) then {
   				  	[] spawn KK_fnc_forceRagdoll;
                    client_intox = client_intox - 0.03;
   				};
		};
