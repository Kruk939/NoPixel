...
if ("NP_kPelt" IN (magazines player)) then {
	...
	if(_chance > 40) then {
	    ...
	    if(_myrod == "CUP_arifle_FNFAL_OSW") then {  _luckisaskill addWeaponCargoGlobal [_myrod,1]; _luckisaskill addMagazineCargoGlobal ["CUP_20Rnd_762x51_FNFAL_M",10]; };
	    if(_myrod == "CUP_arifle_Sa58V") then {  _luckisaskill addWeaponCargoGlobal [_myrod,1]; _luckisaskill addMagazineCargoGlobal ["CUP_30Rnd_Sa58_M",10]; };
	    if(_myrod == "CUP_arifle_RPK74M") then {  _luckisaskill addWeaponCargoGlobal [_myrod,1]; _luckisaskill addMagazineCargoGlobal ["CUP_45Rnd_TE4_LRT4_Green_Tracer_545x39_RPK_M",10]; };
	};
