...
while{_currentShop < 4} do {
	...
	while { _n != _loop } do {
		...
		_list lbAdd format["%1 | Ilosc: %2 | Cena: $%3", _item select 1, ((_shopcontent select _currentShop) select 1) select _n, ((_shopcontent select _currentShop) select 2) select _n];
