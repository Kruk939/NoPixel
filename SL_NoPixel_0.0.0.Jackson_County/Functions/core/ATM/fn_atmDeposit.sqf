...
if (_amount > 0) then 
{
	...
	if(_bankType == 0) then {
		...
		if (_check) then 
		{
			...
			[player, "ATMDeposit", format["Gracz wplacil %1",amount]] remoteExec ["Server_fnc_insertLog", 2];
		...
		if (_check) then 
		{
			...
			[_amount] call Client_fnc_removeCash;
			["Sukces","Pomyślnie wpłacono pieniądze",[0,255,0,1],""] call Client_fnc_showNotification;
			[player, "MafiaATMDeposit", format["Gracz wplacil %1",amount]] remoteExec ["Server_fnc_insertLog", 2];
			//hint "Pomyślnie wpłacono pieniądze!";
