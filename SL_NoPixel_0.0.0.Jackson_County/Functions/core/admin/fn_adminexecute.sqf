...
if (_statementsent isEqualTo 1) then {
...
if (_statementsent isEqualTo 2) then {
...
if(_statementsent isEqualTo 12) then {
hint "Function disabled";
};

if(_statementsent isEqualTo 13) then {
	[] remoteExec ["client_fnc_fullheal", _target]; 
};

if(_statementsent isEqualTo 14) then {
	[_target] remoteExec ["client_fnc_revived", _target]; 
};

if(_statementsent isEqualTo 15) then {
	if(vehicle _TARGET != _target) then {
		[(vehicle _TARGET)] spawn client_fnc_moveIn;
	};
};

if(_statementsent isEqualTo 16) then {
	[_target] remoteExec ["client_fnc_revived", _target]; 
};
...
if(_statementsent isEqualTo 17) then {
	private["_crew"];
...
if(_statementsent isEqualTo 18) then {
...
if(_statementsent isEqualTo 19) then {
...
if(_statementsent isEqualTo 20) then {
	_units = nearestObjects [player, ["Man"], 300];
	{
...
if(_statementsent isEqualTo 21) then {
	_units = nearestObjects [player, ["Man"], 300];
	{
...
if(_statementsent isEqualTo 22) then {
	{
		[_x] remoteExec ["client_fnc_revived",_x];
	} foreach allplayers;
};

if(_statementsent isEqualTo 23) then {
	params ["_veh"];
	_veh = vehicle _target;
	_locked = locked _veh;
...
if(_statementsent isEqualTo 24) then {
...
if(_statementsent isEqualTo 25) then {
	["seagull", getpos player] remoteExec ["createvehicle",-2];
};


if(_statementsent isEqualTo 26) then {
...
if(_statementsent isEqualTo 27) then {
