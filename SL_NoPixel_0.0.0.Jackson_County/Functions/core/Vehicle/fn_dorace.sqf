...
while{true} do {
	...
	if(player distance getmarkerpos _marker < 40) then {
		...
		if(_markerNum isEqualTo 36) then { _markerNum = 1;  };

		if(_markerNum isEqualTo 1) then { _laps = _laps + 1; _lapTime = _timer;  if(_laptime < _myfastest) then { _myfastest = _laptime; }; hint format["Your last laptime was: %1 seconds --- fastest: %2",_laptime,_myfastest];  _timer = 0; };

		if(_laps isEqualTo 3) then { _finish = true; hint "Ukonczyles sesje i otrzymales $175!"; [175] call Client_fnc_addCash; };
