...
while{true} do {
	...
	if(player distance getmarkerpos _marker < 35) then {
		...
		if(_markerNum isEqualTo 15) then { _markerNum = 1;  };

		if(_markerNum isEqualTo 1) then { _laps = _laps + 1; _lapTime = _timer;  if(_laptime < _myfastest) then { _myfastest = _laptime; }; hint format["Twoje ostatnie okrążenie to: %1 sekund --- Najszybsze: %2",_laptime,_myfastest];  _timer = 0; };

		if(_laps isEqualTo 3) then { _finish = true; hint "Ukonczyłes sesje i otrzymales $100!"; [100] call Client_fnc_addCash; };
